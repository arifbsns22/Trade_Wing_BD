...
class VendorController extends GetxController {
  ...
  String get vendorMobile => _authController.currentUserMobile.value.trim();
  ...
  final RxList<OrderModel> vendorOrders = <OrderModel>[].obs;
  ...
  int get pendingOrdersCount =>
      vendorOrders.where((o) => o.orderStatus == OrderStatus.pending).length;
  int get completedOrdersCount =>
      vendorOrders.where((o) => o.orderStatus == OrderStatus.delivered).length;

  @override
  void onInit() {
    super.onInit();
    if (vendorMobile.isNotEmpty) {
      setupListeners();
    } else {
  ...
  void listenToProducts() {
    if (vendorMobile.isEmpty) return;
    isLoadingProducts.value = true;
    _firestore
        .collection('products')
        .where('isResellerProduct', isEqualTo: true)
        .where('resellerMobile', isEqualTo: vendorMobile)
        .snapshots()
        .listen((snapshot) {
      vendorProducts.value = snapshot.docs.map((doc) {
  ...
  void listenToOrders() {
    ...
    _firestore
        ...
        .snapshots()
        .listen((snapshot) {
      vendorOrders.value = snapshot.docs.map((doc) {
  ...
  Future<bool> addVendorProduct(Product product) async {
    try {
      isSubmittingAction.value = true;

      // Get shop name
      final userDoc = await _firestore.collection('users').doc(vendorMobile).get();
      String shopNameVal = '';
      if (userDoc.exists) {
      ...
      data['resellerMobile'] = vendorMobile;
      data['status'] = 'public';

      final customProductId = 'reseller_${vendorMobile}_${DateTime.now().millisecondsSinceEpoch}';
  ...
  Future<bool> updateVendorProduct(String productId, Product product) async {
    try {
      isSubmittingAction.value = true;
  ...
  Future<bool> toggleVendorProductStatus(String productId, String currentStatus) async {
    try {
      isSubmittingAction.value = true;
  ...
  Future<bool> requestWithdrawal({
    ...
    if (vendorMobile.isEmpty) return false;
    try {
      isSubmittingAction.value = true;
