...
class VendorController extends GetxController {
  ...
  RxInt get pendingOrdersCount => 
      vendorOrders.where((o) => o.orderStatus == OrderStatus.pending).length.obs;
      
  RxInt get completedOrdersCount => 
      vendorOrders.where((o) => o.orderStatus == OrderStatus.delivered).length.obs;

  RxDouble get totalProfit => () {
    double profit = 0.0;
    for (var doc in vendorOrders) {
      if (doc.orderStatus == OrderStatus.delivered) {
        // We will fetch the vendorProfit directly from order metadata if it exists
        // Since OrderModel fromMap doesn't parse vendorProfit, we will query it dynamically below
      }
    }
    return profit;
  }().obs;
  ...
  void listenToOrders() {
    ...
    _firestore
        .collection('orders')
        .where('vendorMobile', isEqualTo: vendorMobile)
        .snapshots()
        .listen((snapshot) {
  ...
  Future<void> _publishToGlobalProducts(String productId, double customPrice) async {
    try {
      ...
      final customProductId = 'vendor_${vendorMobile}_$productId';
