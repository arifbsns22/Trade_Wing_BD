...
class AdminVendorListTable extends StatelessWidget {
                  ...
                  child: Skeletonizer(
                    ...
                    child: DataTable(
                      ...
                      columns: const [
                        ...
                        DataColumn(label: Text('ভেন্ডর অর্জিত লাভ')),
                      ...
                      rows: displayList.asMap().entries.map((entry) {
                        ...
                        final status = reseller['vendorVerificationStatus'] ?? 
                            (reseller['role']?.toString().toLowerCase().trim() == 'vendor' ? 'approved' : 'none');
                            ...
                            DataCell(Row(
                              ...
                              children: [
                                IconButton(
                                  ...
                                  onPressed: () => _showVendorDetailDialog(context, reseller, controller, status),
  ...
  void _confirmStatusChange(BuildContext context, String mobile, String newStatus, AdminVendorDashboardController controller) {
  ...
  void _showVendorDetailDialog(BuildContext context, Map<String, dynamic> vendor, AdminVendorDashboardController controller, String currentStatus) {
    final categories = vendor['vendorCategories'] ?? 'উল্লেখ করা হয়নি';
    final nidNumber = vendor['vendorNidNumber'] ?? 'উল্লেখ করা হয়নি';
    final tradeLicense = vendor['vendorTradeLicense'] ?? 'উল্লেখ করা হয়নি';
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                // Text Info
                _buildDetailRow('নাম', vendor['name'] ?? 'Unnamed'),
                _buildDetailRow('শপের নাম', vendor['shopName'] ?? 'No Shop Name'),
                ...
                Row(
                  children: [
                    Expanded(child: _buildImagePreview('NID Front', vendor['vendorNidFront'])),
                    const SizedBox(width: 10),
                    Expanded(child: _buildImagePreview('NID Back', vendor['vendorNidBack'])),
                  ],
                ),
                const SizedBox(height: 10),
                _buildImagePreview('Selfie / নিজের ছবি', vendor['vendorSelfie']),
